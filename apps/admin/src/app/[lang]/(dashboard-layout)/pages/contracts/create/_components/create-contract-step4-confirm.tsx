      ...
      <Card className="border-emerald-200 dark:border-emerald-950 shadow-md">
        <CardHeader className="bg-emerald-50/50 dark:bg-emerald-950/20 border-b border-emerald-100 dark:border-emerald-900/40">
          <CardTitle className="text-lg flex items-center gap-2 text-emerald-800 dark:text-emerald-300">
            <ShieldCheck className="w-5 h-5 text-emerald-600" /> {t("contracts.wizard.step4Title")}
          </CardTitle>
          <CardDescription>
            {t("contracts.wizard.step4Notice")}
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-900/70 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 text-sm">
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.contractTitleLabel")}</span>
              <span className="font-bold text-slate-900 dark:text-white">{title}</span>
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.sourceLabel")}</span>
              <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200">
                {t("contracts.wizard.manualSource")}
              </Badge>
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.customerLabel")}</span>
              <span className="font-semibold text-slate-900 dark:text-white">
                {selectedUser?.name || selectedUser?.username} ({selectedUser?.email})
              </span>
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.ekycLabel")}</span>
              {selectedUser?.isVerified ? (
                <span className="inline-flex items-center gap-1 text-xs font-bold text-emerald-700">
                  <UserCheck className="w-3.5 h-3.5" /> {t("contracts.wizard.ekycVerified")}
                </span>
              ) : (
                <span className="text-xs text-amber-600">{t("contracts.wizard.ekycUnverified")}</span>
              )}
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.totalValueLabel")}</span>
              <span className="text-base font-extrabold text-primary">
                {contractValue.toLocaleString("vi-VN")} VNĐ
              </span>
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.paymentStatusLabel")}</span>
              <Badge
                variant="outline"
                className={
                  paymentStatus === "paid"
            ...
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.treeCodeLabel")}</span>
              <span className="font-mono text-xs">
                {selectedTreeCode !== "none" ? selectedTreeCode : t("contracts.wizard.managedByQuantity")}
              </span>
            </div>
            <div>
              <span className="text-xs text-muted-foreground block">{t("contracts.wizard.validityLabel")}</span>
          ...
          <div className="space-y-2.5">
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-xs font-medium text-emerald-700 bg-emerald-50/70 dark:bg-emerald-950/30 p-2 rounded-lg border border-emerald-200 dark:border-emerald-900">
                ...
                <span>{t("contracts.wizard.condCustomerValid", { email: selectedUser?.email || "" })}</span>
              </div>
              <div className="flex items-center gap-2 text-xs font-medium text-emerald-700 bg-emerald-50/70 dark:bg-emerald-950/30 p-2 rounded-lg border border-emerald-200 dark:border-emerald-900">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
                <span>{t("contracts.wizard.condValueValid", { value: contractValue.toLocaleString("vi-VN") })}</span>
              </div>
              <div className="flex items-center gap-2 text-xs font-medium text-emerald-700 bg-emerald-50/70 dark:bg-emerald-950/30 p-2 rounded-lg border border-emerald-200 dark:border-emerald-900">
                <CheckCircle2 className="w-4 h-4 text-emerald-600 shrink-0" />
        ...
        <CardFooter className="bg-slate-50 dark:bg-slate-900 p-6 flex flex-col sm:flex-row items-center justify-between gap-4 border-t border-slate-200 dark:border-slate-800">
          <Button variant="outline" onClick={onBackToStep3}>
            <ArrowLeft className="w-4 h-4 mr-1.5" /> {t("contracts.wizard.backToContent")}
          </Button>
          <div className="flex items-center gap-3 w-full sm:w-auto">
            ...
            >
              <Save className="w-4 h-4 mr-1.5" /> {t("contracts.wizard.saveDraft")}
            </Button>
            <Button
            ...
            >
              <Send className="w-4 h-4 mr-1.5" /> {isSubmitting ? t("contracts.wizard.signing") : t("contracts.wizard.publishContract")}
            </Button>
          </div>
        </CardFooter>
      </Card>
