...
import { Sprout, Plus, Thermometer, Droplets, RefreshCw } from 'lucide-react';
...
export default function CareLogsPage() {
  ...
  const { data: response, isLoading, isError, refetch } = useApiQuery<CareLog[] | { items: CareLog[] }>(
    ['care-logs'],
  ...
  return (
    <RoleGuard allowedRoles={['SUPER_ADMIN', 'ADMIN']}>
      <div className="container mx-auto p-4 md:p-6 space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Ghi nhận các hoạt động tưới nước, bón phân, đo đạc thông số môi trường từ REST API
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-base font-semibold">Lịch sử Nhật ký Chăm sóc Thực tế</CardTitle>
          </CardHeader>
          <CardContent>
            {isLoading ? (
              <div className="py-8 text-center text-sm text-muted-foreground">Đang tải nhật ký từ API...</div>
            ) : isError ? (
              ...
              <div className="py-8 text-center text-sm text-muted-foreground">
                Chưa có nhật ký chăm sóc nào được tạo. Nhấn "Nhập Nhật ký Mới" để tạo nhật ký đầu tiên!
              </div>
            ) : (
              <Table>
                <TableHeader>
                  ...
                  {careLogs.map((log) => (
                    <TableRow key={log.id}>
                      <TableCell className="font-medium">
                        {log.treeCode || log.bedCode || log.gardenName || log.id}
                      </TableCell>
                      <TableCell>
                        ...
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          {log.temperature && (
                            <span className="flex items-center gap-1">
                              <Thermometer className="w-3.5 h-3.5 text-amber-500" /> {log.temperature}°C
                            </span>
                          )}
                          {log.humidity && (
                            <span className="flex items-center gap-1">
                              <Droplets className="w-3.5 h-3.5 text-blue-500" /> {log.humidity}%
                            </span>
                          )}
                        </div>
                      </TableCell>
                      <TableCell className="max-w-md text-sm">{log.description || log.notes || '—'}</TableCell>
