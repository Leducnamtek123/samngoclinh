...
export function useGardensManager({
  initialGardens,
  initialError,
  ...
  const handleToggleAll = () => {
    ...
    const isAllSelected = allFilteredIds.every((id) =>
      new Set(selectedGardenIds).has(id)
    )
  ...
  const handleOpenEdit = (garden: Garden) => {
    setDialogState({
      ...
      formData: {
        ...
        area:
          garden.area !== undefined && garden.area !== null
            ? String(garden.area)
            : "",
        latitude:
          garden.latitude !== undefined && garden.latitude !== null
            ? String(garden.latitude)
            : "",
        longitude:
          garden.longitude !== undefined && garden.longitude !== null
            ? String(garden.longitude)
            : "",
        managerName: garden.managerName || "",
        managerPhone: garden.managerPhone || "",
        establishedAt: garden.establishedAt
          ? garden.establishedAt.substring(0, 10)
          : "",
        maxBeds:
          garden.maxBeds !== undefined && garden.maxBeds !== null
            ? String(garden.maxBeds)
            : "",
      },
    })
  }
    ...
    try {
      ...
      if (dialogState.mode === "create") {
        ...
        const res = await fetchApi(
          `/user/cultivation/gardens/${dialogState.selectedGarden.id}`,
          {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
  ...
  const performBulkDelete = async () => {
    ...
    if (successCount > 0) {
      setGardens((prev) => {
        const set = new Set(selectedGardenIds)
