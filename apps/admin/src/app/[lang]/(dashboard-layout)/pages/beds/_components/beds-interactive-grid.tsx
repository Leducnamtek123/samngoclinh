...
const getHealthBadge = (health: string) => {
  switch (health?.toLowerCase()) {
    case "healthy":
      return (
        <Badge className="bg-emerald-500/10 text-emerald-600 border-transparent text-[10px] font-bold">
          Khỏe mạnh
        </Badge>
      )
    case "sick":
      return (
        <Badge className="bg-amber-500/10 text-amber-600 border-transparent text-[10px] font-bold">
          Cần tưới nước
        </Badge>
      )
    case "dead":
      return (
        <Badge className="bg-red-500/10 text-red-650 border-transparent text-[10px] font-bold">
          Sâu bệnh
        </Badge>
      )
    default:
...
const handleDragStart = (
  e: React.DragEvent,
  sourceLoc: CultivationBedLocation
) => {
  e.dataTransfer.setData("application/json", JSON.stringify(sourceLoc))
}

export function BedsInteractiveGrid({
  tableData,
  gardens,
      ...
      <div className="p-4 border-b border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge
              variant="outline"
              className="bg-emerald-50 text-emerald-600 border-emerald-200 dark:bg-emerald-950/20 text-[9px] font-bold py-0.5 rounded-md"
            >
              Active
            </Badge>
          </div>
        ...
        <div className="flex flex-wrap gap-2 text-xs md:mr-auto md:ml-6">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 border rounded-lg px-2.5 py-1 flex flex-col items-center justify-center min-w-[90px]">
            ...
            <span className="font-bold text-slate-700 dark:text-slate-300">
              {activeBed.treeCount}/{activeBed.maxTrees || 100}
            </span>
          </div>
          <div className="bg-slate-50 dark:bg-slate-800/60 border rounded-lg px-2.5 py-1 flex flex-col items-center justify-center min-w-[70px]">
            <span className="text-[9px] text-slate-400 uppercase tracking-wider font-semibold">
          ...
          <div className="bg-slate-50 dark:bg-slate-800/60 border rounded-lg px-2.5 py-1 flex flex-col items-center justify-center min-w-[80px]">
            ...
            <span className="font-bold text-emerald-600">
              {formatDaysAgo(activeBed.lastWateredAt)}
            </span>
          </div>
        ...
        <div className="flex items-center gap-1.5 self-end md:self-auto">
          <Button
            size="sm"
            variant="outline"
            className="h-8 text-xs font-semibold px-2.5 border-slate-200"
            onClick={() => openEditDialog(activeBed)}
          >
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 bg-white dark:bg-slate-950 p-2 px-3 rounded-xl border border-slate-200/80 dark:border-slate-800 shadow-xxs">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          value={gridCustomerFilter}
          onValueChange={setGridCustomerFilter}
        >
        ...
        <div className="flex items-center gap-1.5 ml-1">
          ...
          />
          <label
            htmlFor="onlyEmptyCheck"
            className="text-[10px] font-bold text-slate-500 select-none"
          >
            Chỉ ô trống
          </label>
        </div>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center p-6 text-center z-10 bg-white/95 dark:bg-slate-900/95 text-slate-500 dark:text-slate-400">
          ...
          <div className="flex items-center gap-1.5 mt-4">
            ...
            <Button
              ...
              size="sm"
              className="bg-emerald-600 hover:bg-emerald-700 h-8 text-[10px] font-bold text-white px-3"
            >
              Khởi tạo lưới
            </Button>
          </div>
                ...
                return (
                  ...
                  >
                    {loc.status === "planted" ? (
                      <>
                        <Leaf
                          className={`h-4.5 w-4.5 ${isSelected ? "text-white" : tree?.healthStatus === "sick" ? "text-amber-600 dark:text-amber-250" : tree?.healthStatus === "dead" ? "text-red-650 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400"}`}
                        />
                        <span
                          className={`text-[7px] font-mono font-bold mt-1 ${isSelected ? "text-white" : "text-slate-700 dark:text-slate-200"}`}
                        >
      ...
      {hoveredCell && (
        ...
        >
          <div className="flex justify-between items-center font-bold">
            <span>
              H{hoveredCell.row + 1} - C{hoveredCell.col + 1}
            </span>
          ...
          {hoveredCell.status === "planted" ? (
            <>
              <div className="text-emerald-400 font-semibold">
                {getCellTree(hoveredCell.treeCode)?.name}
              </div>
              <div className="text-slate-400 truncate">
                Chủ:{" "}
                {getOwnerName(getCellTree(hoveredCell.treeCode)?.ownerUserId)}
              </div>
      ...
      {locations.length > 0 && (
        <div className="absolute bottom-4 right-4 bg-white/90 dark:bg-slate-900/90 border border-slate-200 dark:border-slate-800 rounded-lg p-1.5 flex items-center gap-1.5 shadow-xl z-20">
          <Button
            ...
            variant="ghost"
            onClick={zoomOut}
            className="h-7 w-7 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white"
          >
            <ZoomOut className="h-3.5 w-3.5" />
          </Button>
          <span className="text-[10px] font-bold font-mono text-slate-500 dark:text-slate-400 w-10 text-center">
            {zoomScale}%
          </span>
          <Button
            size="icon"
          ...
          {bedTrees.length === 0 ? (
            ...
            bedTrees.map((tree: Tree) => (
              <TableRow key={tree.id} className="hover:bg-slate-50/20">
                ...
                <TableCell className="text-slate-600 text-xs font-bold">
                  {tree.ageYear} năm tuổi
                </TableCell>
                <TableCell className="text-slate-600 text-xs font-medium">
                  {getOwnerName(tree.ownerUserId)}
                </TableCell>
                <TableCell className="font-mono text-slate-700 text-xs font-bold">
                  {formatVND(tree.priceBought || 0)}
                </TableCell>
                <TableCell className="text-slate-500 font-mono text-xs">
                  {tree.plantedAt
                    ? new Date(tree.plantedAt).toLocaleDateString("vi-VN", {
      ...
      <div className="bg-white dark:bg-slate-900 border rounded-xl p-4 shadow-xxs space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="flex flex-col gap-1">
            ...
            <span className="font-bold text-amber-600">
              {formatDaysAgo(activeBed.lastFertilizedAt)}
            </span>
          </div>
        </div>
        {activeBed.description && (
          <div className="pt-2 border-t text-xs">
            ...
            <p className="text-slate-650 dark:text-slate-400 leading-relaxed font-semibold italic">
              {activeBed.description}
            </p>
          </div>
        )}
      ...
      <div className="bg-white dark:bg-slate-900 border rounded-xl p-4 shadow-xxs space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3 text-xs">
          ...
          <div className="bg-emerald-50/50 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-350 rounded-lg p-2.5 border border-emerald-100/80 dark:border-emerald-900/50">
            <span className="text-emerald-600 dark:text-emerald-500 text-[10px] block font-bold">
              KHỎE MẠNH (HEALTHY)
            </span>
          ...
          <div className="bg-red-50/50 dark:bg-red-950/20 text-red-800 dark:text-red-350 rounded-lg p-2.5 border border-red-100/80 dark:border-red-900/50">
            <span className="text-red-650 dark:text-red-500 text-[10px] block font-bold">
              CẦN CHĂM SÓC / HẠI
            </span>
            <span className="font-bold text-base mt-1 block">
    ...
    <div className="flex-1 bg-white dark:bg-slate-900 border rounded-xl overflow-hidden shadow-xxs flex flex-col">
      <div className="p-3 border-b bg-slate-50/50 dark:bg-slate-900/50 flex items-center justify-between border-slate-100 dark:border-slate-800">
        <span className="text-xs font-bold text-slate-650 dark:text-slate-350">
          Nhật ký lịch sử canh tác luống
        </span>
      </div>
      <div className="flex-1 overflow-y-auto p-4">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-400 dark:text-slate-500 font-semibold text-xs">
            Chưa ghi nhận hoạt động chăm sóc nào tại luống này.
          </div>
        ) : (
