...
export function ToastCard({
  ...
  return (
    ...
    >
      <div className={cn("w-9 h-9 rounded-full flex items-center justify-center flex-shrink-0", currentStyle.iconContainer)}>
        {currentStyle.icon}
      </div>
          ...
          <div className="flex-grow overflow-y-auto divide-y divide-slate-100 dark:divide-slate-850">
            {notifications.length === 0 ? (
              <div className="py-12 px-4 flex flex-col items-center justify-center text-center text-slate-400">
                ...
                <p className="text-xs text-slate-500 mt-1">Chúng tôi sẽ thông báo cho bạn khi có cập nhật mới.</p>
              </div>
            ) : (
              notifications.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className={cn("w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5", colors[item.type])}>
                      {icons[item.type]}
                    </div>
                    <div className="flex-grow min-w-0 pr-4">
                      <p className={cn("text-xs leading-snug text-slate-800 dark:text-slate-200", !item.isRead ? "font-bold" : "font-medium")}>
                        {item.title}
                      </p>
...
export function TableSkeleton({ rows = 5, cols = 4 }: { rows?: number; cols?: number }) {
  return (
    <div className="w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl overflow-hidden shadow-sm">
      {/* Table Header */}
      <div className="bg-slate-50/80 dark:bg-slate-900/50 p-4 border-b border-slate-100 dark:border-slate-850 grid gap-4" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
        {Array.from({ length: cols }).map((_, i) => (
          <Skeleton key={i} className="h-4 w-3/4 bg-slate-200/80 dark:bg-slate-800" />
        ))}
      </div>
      {/* Table Rows */}
      <div className="p-4 flex flex-col gap-6">
        {Array.from({ length: rows }).map((_, r) => (
          <div key={r} className="grid gap-4 items-center" style={{ gridTemplateColumns: `repeat(${cols}, minmax(0, 1fr))` }}>
            <div className="flex items-center gap-3">
              <Skeleton className="w-10 h-10 rounded-lg flex-shrink-0" />
              <Skeleton className="h-3 w-full" />
            </div>
            {Array.from({ length: cols - 1 }).map((_, c) => (
              <Skeleton key={c} className={cn("h-3", c === cols - 2 ? "w-1/2" : "w-3/4")} />
            ))}
          </div>
        ))}
      </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white/95 dark:bg-slate-900/95 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-2xl p-6 z-10 animate-scale-up">
        ...
        <div className="flex items-start gap-4 mb-4">
          ...
          <div>
            <h3 className="font-semibold text-lg text-slate-900 dark:text-slate-100 leading-snug">{title}</h3>
        ...
        <div className="flex gap-3 justify-end mt-8">
          ...
          <button
            ...
            className={cn(
              ...
              type === "danger" ? "bg-red-600 hover:bg-red-700" : "bg-emerald-700 hover:bg-emerald-800"
            )}
          >
...
export function ProgressIndicator({
  ...
  return (
    <div className={cn("bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-2xl p-5 shadow-sm w-full max-w-sm", className)}>
      {progress !== undefined && (
        <div className="mb-4">
          <div className="flex justify-between items-center mb-2">
            <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{label || "Đang tải tiến trình..."}</span>
            <span className="text-xs font-bold text-slate-900 dark:text-slate-100">{Math.round(progress)}%</span>
          </div>
      ...
      {isSyncing && (
        <div className="flex items-center gap-3 text-emerald-700 dark:text-emerald-400">
          ...
          <span className="text-xs font-semibold">{label || "Đang đồng bộ dữ liệu..."}</span>
        </div>
      )}
    </div>
  )
