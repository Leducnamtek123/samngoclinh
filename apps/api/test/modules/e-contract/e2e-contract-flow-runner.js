...
async function runE2EContractTests() {
    ...
    const mockEContractRepo = {
        ...
        signContract: async (id, signatureUrl, pdfUrl, metadata) => {
            const existing = db.contracts.get(id);
            if (!existing) throw new NotFoundException('Not found');
            const signed = {
                ...existing,
    ...
    const mockPdfService = {
        generateSignedContractPdf: async (params) => {
    ...
    const mockFileService = {
        uploadBuffer: async (_buffer, folder, filename) => {
            return `https://cdn.samngoclinh.vn/${folder}/${filename}`;
        },
        uploadBase64: async (_base64, folder) => {
    ...
    assert(
        signResult.data?.metadata?.documentHash?.length === 64,
    ...
    try {
        await contractService.updateContract(createdContract.id, { title: 'Illegal Edit' });
    } catch (err) {
