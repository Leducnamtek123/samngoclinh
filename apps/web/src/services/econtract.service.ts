...
export const econtractService = {
  ...
  async signContract(
    ...
    return await fetchApiClient(`/user/contracts/${id}/sign`, {
      method: 'POST',
  ...
  async renewContract(
    ...
    return await fetchApiClient(`/user/contracts/${id}/renew`, {
      method: 'POST',
  ...
  async verifyContract(code: string): Promise<ContractVerifyResponse> {
    const res = await fetchApiClient(`/public/contracts/verify/${encodeURIComponent(code)}`);
    return (res?.data || res) as ContractVerifyResponse;
  },
};
