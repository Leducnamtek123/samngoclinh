...
async function getMarketplaceData() {
  ...
  try {
    const listRes = await fetchApi('/public/marketplace/listings', { cache: 'no-store' });
    if (listRes.ok) {
...
export default async function TradingFloorPage(props: TradingFloorPageProps) {
  ...
  const { listings, profile } = await getMarketplaceData();
      ...
      <section className="max-w-7xl mx-auto px-4 md:px-8 py-8 space-y-6">
        ...
        {!isVerified && (
          <div className="bg-red-50 border border-red-200 rounded-2xl p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 shadow-sm">
            ...
            <Link href="/profile?tabs=kyc" className="bg-white hover:bg-gray-50 text-red-700 border border-red-200 px-5 py-2.5 rounded-lg text-xs font-bold shadow-sm transition-colors whitespace-nowrap">
              Xác thực ngay
            </Link>
          </div>
        )}
                  ...
                  <div key={listing.id} className="px-6 py-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 hover:bg-gray-50/50 transition-all">
                    ...
                    <div className="flex items-center gap-6 justify-between sm:justify-end w-full sm:w-auto">
                      <div className="text-right">
                        <p className="font-bold text-gray-900 text-base">{listing.price.toLocaleString('vi-VN')} d</p>
                        {/* Payment badges */}
