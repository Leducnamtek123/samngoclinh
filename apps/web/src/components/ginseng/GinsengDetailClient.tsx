  ...
  if (isError || !plant) {
    return (
      <div className="max-w-4xl mx-auto py-20 px-4 text-center space-y-4">
        ...
        <p className="text-gray-500 text-sm">{tProd('notFoundDesc')}</p>
        <Link
          href={`/${locale}/ginseng`}
    ...
    <div className="w-full bg-gray-50 min-h-screen py-10 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <nav className="flex items-center gap-2 text-xs text-gray-500 font-medium">
          <Link href={`/${locale}`} className="hover:text-gray-900 transition-colors">{tNav('home')}</Link>
          <span>/</span>
          <Link href={`/${locale}/ginseng`} className="hover:text-gray-900 transition-colors">{tNav('ginseng')}</Link>
          <span>/</span>
            ...
            {images.length > 1 && (
              <div className="flex gap-3 overflow-x-auto py-1">
                {images.map((imgUrl: string, idx: number) => (
                  ...
                  >
                    <Image src={imgUrl} alt={`Thumbnail ${idx + 1}`} fill sizes="64px" unoptimized className="object-cover" />
                  </button>
                ))}
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              {plant.code && (
                <p className="text-xs text-gray-400 font-mono">
                  {tProd('productCode')}: <span className="text-gray-700 font-semibold">{plant.code}</span>
                </p>
              )}
              ...
              <div className="grid grid-cols-2 gap-3 bg-gray-50 p-4 rounded-2xl border border-gray-100 text-xs">
                <div>
                  <span className="text-gray-400 font-medium block">{tTrees('age')}:</span>
                  <span className="font-bold text-gray-800">{plant.ageYears ? `${plant.ageYears}` : '—'}</span>
                </div>
                <div>
                  <span className="text-gray-400 font-medium block">{tTrees('garden')}:</span>
                  <span className="font-bold text-gray-800">{plant.gardenLocation || 'Nam Trà My, Kon Tum'}</span>
                </div>
                <div>
                  <span className="text-gray-400 font-medium block">{tTrees('health')}:</span>
                  <span className="font-bold text-emerald-700">{tTrees('healthy')}</span>
                </div>
                <div>
                  <span className="text-gray-400 font-medium block">{tProd('guaranteedQuality')}:</span>
                  <span className="font-bold text-emerald-700">100% DNA Certified</span>
                </div>
              </div>

              <div className="border-t border-gray-100 pt-4 space-y-2">
                <h4 className="text-xs font-bold uppercase text-gray-400 tracking-wider">{tProd('description')}</h4>
              ...
              <div className="space-y-2 pt-2">
                <span className="text-xs font-bold uppercase text-gray-400 tracking-wider block">{tProd('quantity')}</span>
