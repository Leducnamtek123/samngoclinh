...
export type NotificationItem = {
  id: string;
  type: 'order' | 'tree' | 'contract' | 'system' | string;
  title: string;
  message: string;
  read: boolean;
  timestamp: string;
...
const NOTIFICATION_I18N: Record<string, Record<string, { title: string; body: string }>> = {
  vi: {
    ...
    verificationEmail: {
      ...
      body: 'Vui lòng xác thực địa chỉ email để kích hoạt tài khoản.',
    },
    verifiedEmail: {
      title: 'Đã xác thực email',
      body: 'Địa chỉ email của bạn đã được xác thực thành công.',
    },
    mobileNumberVerified: {
      title: 'Đã xác thực số điện thoại',
      body: 'Số điện thoại của bạn đã được xác thực thành công.',
    },
    changePassword: {
    ...
    forgotPassword: {
      title: 'Quên mật khẩu',
      body: 'Chúng tôi đã nhận được yêu cầu đặt lại mật khẩu của bạn.',
    },
    publishTermPolicy: {
      title: 'Cập nhật điều khoản chính sách',
      body: 'Điều khoản chính sách đã có bản cập nhật mới. Vui lòng xem lại.',
    },
    newDeviceLogin: {
      title: 'Đăng nhập thiết bị mới',
      body: 'Phát hiện một lượt đăng nhập mới vào tài khoản của bạn từ thiết bị mới.',
    },
    resetTwoFactorByAdmin: {
      title: 'Đặt lại xác thực hai lớp',
      body: 'Xác thực hai lớp của bạn đã được đặt lại.',
    },
    temporaryPasswordByAdmin: {
      title: 'Mật khẩu tạm thời',
      body: 'Một mật khẩu tạm thời đã được thiết lập cho tài khoản.',
    },
    resetPassword: {
      title: 'Đặt lại mật khẩu',
      body: 'Mật khẩu của bạn đã được đặt lại thành công.',
    },
    userAcceptTermPolicy: {
      title: 'Đã chấp nhận điều khoản chính sách',
      body: 'Bạn đã chấp nhận phiên bản mới của điều khoản chính sách.',
    },
  },
  en: {
    ...
    publishTermPolicy: {
      title: 'Term Policy Update',
      body: 'A new term policy version has been published.',
    },
    newDeviceLogin: {
      title: 'New Device Login',
      body: 'New device login detected on your account.',
    },
    resetTwoFactorByAdmin: {
      title: 'Reset Two-Factor Authentication',
      body: 'Your two-factor authentication has been reset.',
    },
    temporaryPasswordByAdmin: {
      title: 'Temporary Password',
      body: 'A temporary password has been set for your account.',
    },
    resetPassword: { title: 'Password Reset', body: 'Your password has been reset successfully.' },
    userAcceptTermPolicy: {
      title: 'Term Policy Accepted',
      body: 'You have accepted the new term policy version.',
    },
  },
};

const formatNotificationString = (
  input: string,
  locale: string,
  type: 'title' | 'body',
): string => {
...
export function useNotificationPopover(
  ...
  const notifications: NotificationItem[] = rawItems.map((item: RawNotification) => ({
    id: String(item.id || item._id || ''),
    type: String(item.type || 'system'),
    title: formatNotificationString(
      String(item.title || item.titleKey || 'System Notification'),
      locale,
      'title',
    ),
    message: formatNotificationString(
      String(item.message || item.bodyKey || item.body || ''),
      locale,
      'body',
    ),
    read: Boolean(item.read || item.isRead),
    timestamp: String(item.timestamp || item.createdAt || new Date().toISOString()),
    details: item.details as NotificationItem['details'],
    orderPayload: item.orderPayload as OrderDetailData | undefined,
  }));
